...
export default function EmailForm() {
  ...
  const usernameEmailDict = dict.findUsername.email;
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const resp = await fetch('/username/email', {
        method: 'post',
  ...
  if (requestState === RequestState.Done) {
    return (
      <section className="border rounded p-2">
        <h2 className="text-h2 mb-2">{usernameEmailDict.emailSent.smallTitle}</h2>
        {usernameEmailDict.emailSent.descriptions.map((desc, index) => (
  ...
  return (
    <section className="border rounded p-2">
      <h2 className="text-h2 mb-2">{dict.title.findUsername}</h2>
      <p>{usernameEmailDict.description}</p>
      <form className="flex flex-row flex-wrap justify-end gap-2 mt-2" onSubmit={handleSubmit}>
        ...
        >
          {usernameEmailDict.buttonSendEmail}
        </Button>
      </form>
    </section>
  );
